...
def _detect_flagged_risks(parsed_symptoms: list, critical_values: list) -> list[dict]:
    ...
    sepsis_matches = []
    if "hypotension" in symptom_names:
        sepsis_matches.append("hypotension")
    if "altered mental status" in symptom_names:
        sepsis_matches.append("altered mental status")
    if "oliguria" in symptom_names:
        sepsis_matches.append("oliguria")
    if _lab_is_high(labs, "lactate", min_value=2.0):
        sepsis_matches.append("lactate > 2 mmol/L")
    if _lab_is_high(labs, "wbc", min_value=12.0):
        sepsis_matches.append("WBC > 12 K/uL")
    if _lab_is_high(labs, "creatinine", min_value=1.2):
        sepsis_matches.append("creatinine elevated")
    if sepsis_matches:
        risks.append(
            {
                ...
                "criteria_met": len(sepsis_matches) >= 3,
                "matched_findings": sepsis_matches,
            }
        )

    shock_matches = []
    if "hypotension" in symptom_names:
        shock_matches.append("persistent hypotension")
    if _lab_is_high(labs, "lactate", min_value=2.0):
        shock_matches.append("lactate > 2 mmol/L")
    if len(shock_matches) >= 2:
        risks.append(
            {
                ...
                "criteria_met": True,
                "matched_findings": shock_matches,
            }
        )

    aki_matches = []
    if "oliguria" in symptom_names:
        aki_matches.append("oliguria")
    if _lab_is_high(labs, "creatinine", min_value=1.5):
        aki_matches.append("creatinine elevated")
    if aki_matches:
        risks.append(
            {
                "risk_flag": "AKI_RISK",
                ...
                "criteria_met": len(aki_matches) >= 1,
                "matched_findings": aki_matches,
            }
        )

    early_warning_matches = []
    if "hypotension" in symptom_names:
        early_warning_matches.append("hypotension")
    if "altered mental status" in symptom_names:
        early_warning_matches.append("altered mental status")
    if early_warning_matches:
        risks.append(
            {
                ...
                "criteria_met": len(early_warning_matches) >= 2,
                "matched_findings": early_warning_matches,
            }
        )
